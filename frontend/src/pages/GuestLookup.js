...
const Icon = {
  ...
  Download: (s=13) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>,
};
...
// ── Persona theme config for poster backgrounds ───────────────
const PERSONA_THEME = {
...
// ── Truncate text to fit in poster ───────────────────────────
function trunc(str, max) {
  if (!str) return "";
  return str.length > max ? str.slice(0, max - 1) + "…" : str;
}

// ── Single SVG Ad Poster ─────────────────────────────────────
function AdPoster({ variant, guestName, persona, meta, analysis }) {
  const personaKey = (persona || "").toLowerCase().replace(" traveler", "").trim();
  const theme = PERSONA_THEME[personaKey] || PERSONA_THEME.default;

  const profession  = meta?.profession   || meta?.jobTitle   || "";
  const company     = meta?.company      || meta?.jobCompany  || "";
  const location    = meta?.location     || "";
  const salary      = meta?.salary       || "";
  const seniority   = meta?.seniority    || "";
  const ageDisplay  = meta?.age ? meta.age + " yrs" : (meta?.ageRange || "");
  const lifestyle   = (meta?.lifestyle   || []).slice(0, 2);
  const spending    = meta?.spendingLevel || "";

  // Each variant shows a different angle
  const variants = {
    1: {
      eyebrow:  "PERSONALISED OFFER",
      headline: trunc(analysis?.personalizedOffer || "Exclusive Guest Package", 55),
      sub:      trunc(analysis?.roomRecommendation || "", 60),
      cta:      "View Offer",
      tag:      theme.label,
      showProfile: true,
    },
    2: {
      eyebrow:  "ROOM RECOMMENDATION",
      headline: trunc(analysis?.roomRecommendation || "Premium Room Selected", 48),
      sub:      trunc(analysis?.staffNote || "", 70),
      cta:      "Book Now",
      tag:      spending ? spending.toUpperCase() + " TIER" : theme.label,
      showProfile: false,
    },
    3: {
      eyebrow:  "GUEST PROFILE MATCH",
      headline: trunc(persona || "Guest Persona", 40),
      sub:      lifestyle.length > 0
        ? "Interests: " + lifestyle.join("  ·  ")
        : trunc(analysis?.staffNote || "", 60),
      cta:      "View Profile",
      tag:      seniority ? seniority.toUpperCase() : theme.label,
      showProfile: true,
    },
  };
  ...
  // Decoration circles for visual depth
  const circles = [
    { cx:340, cy:30,  r:90,  opacity:0.07 },
    { cx:380, cy:120, r:55,  opacity:0.05 },
    { cx:20,  cy:160, r:70,  opacity:0.06 },
  ];
  ...
  return (
    ...
    >
      <defs>
        ...
        <linearGradient id={"shine_" + variant} x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="0%"   stopColor="#ffffff" stopOpacity="0.04" />
          <stop offset="100%" stopColor="#ffffff" stopOpacity="0"    />
        </linearGradient>
        <clipPath id={"clip_" + variant}>
          <rect width="420" height="220" rx="10" />
        </clipPath>
      </defs>

      <g clipPath={"url(#clip_" + variant + ")"}>
        {/* Background */}
        <rect width="420" height="220" fill={"url(#grad_" + variant + ")"} />
        <rect width="420" height="220" fill={"url(#shine_" + variant + ")"} />

        {/* Decoration circles */}
        {circles.map((c, i) => (
          <circle key={i} cx={c.cx} cy={c.cy} r={c.r}
            fill={theme.accent} opacity={c.opacity} />
        ))}

        {/* Left accent bar */}
        <rect x="0" y="0" width="4" height="220" fill={theme.accent} opacity="0.9" />

        {/* Top-right tag pill */}
        <rect x="320" y="14" width={v.tag.length * 7 + 18} height="20" rx="4"
          fill={theme.accent} opacity="0.18" />
        <text x="329" y="27" fontSize="8" fontWeight="700" fill={theme.accent}
          fontFamily="'DM Sans','Helvetica Neue',sans-serif" letterSpacing="0.08em">
          {v.tag}
        </text>

        {/* Eyebrow */}
        <text x="22" y="38" fontSize="8" fontWeight="700" fill={theme.accent}
          fontFamily="'DM Sans','Helvetica Neue',sans-serif" letterSpacing="0.12em" opacity="0.9">
          {v.eyebrow}
        </text>

        {/* Divider line under eyebrow */}
        <line x1="22" y1="44" x2="200" y2="44" stroke={theme.accent} strokeWidth="0.5" opacity="0.3" />

        {/* Headline — wrapped across 2 lines max */}
        {(() => {
          ...
          return lines.slice(0, 2).map((line, i) => (
            <text key={i} x="22" y={68 + i * 22} fontSize="17" fontWeight="700"
              fill="#ffffff" fontFamily="'DM Sans','Helvetica Neue',sans-serif"
              letterSpacing="-0.3px">
              {line}
            </text>
          ));
        ...
        {/* Sub text */}
        {v.sub && (
          <text x="22" y="122" fontSize="10" fill="#ffffff"
            fontFamily="'DM Sans','Helvetica Neue',sans-serif" opacity="0.65"
            style={{ whiteSpace:"pre-wrap" }}>
            <tspan x="22" dy="0">{trunc(v.sub, 52)}</tspan>
            {v.sub.length > 52 && (
              <tspan x="22" dy="14">{trunc(v.sub.slice(52), 52)}</tspan>
            )}
          </text>
        )}

        {/* Profile info row (variant 1 and 3) */}
        {v.showProfile && (
          <>
            <line x1="22" y1="150" x2="398" y2="150" stroke="#ffffff" strokeWidth="0.4" opacity="0.12" />
            <text x="22" y="165" fontSize="9" fill="#ffffff"
              fontFamily="'DM Sans','Helvetica Neue',sans-serif" opacity="0.5">
              GUEST
            </text>
            <text x="60" y="165" fontSize="9" fontWeight="600" fill="#ffffff"
              fontFamily="'DM Sans','Helvetica Neue',sans-serif" opacity="0.85">
              {trunc(guestName || "Guest", 20)}
            </text>

            {profession && (
              <>
                <text x="180" y="165" fontSize="9" fill="#ffffff"
                  fontFamily="'DM Sans','Helvetica Neue',sans-serif" opacity="0.5">
                  ROLE
                </text>
                <text x="208" y="165" fontSize="9" fontWeight="600" fill="#ffffff"
                  fontFamily="'DM Sans','Helvetica Neue',sans-serif" opacity="0.85">
                  {trunc(profession, 18)}
                </text>
              </>
            ...
            {(ageDisplay || location) && (
              <>
                <text x="22" y="180" fontSize="9" fill="#ffffff"
                  fontFamily="'DM Sans','Helvetica Neue',sans-serif" opacity="0.5">
                  {ageDisplay ? "AGE" : "LOC"}
                </text>
                <text x="60" y="180" fontSize="9" fontWeight="600" fill="#ffffff"
                  fontFamily="'DM Sans','Helvetica Neue',sans-serif" opacity="0.85">
                  {ageDisplay || location}
                </text>
                {location && ageDisplay && (
                  <>
                    <text x="180" y="180" fontSize="9" fill="#ffffff"
                      fontFamily="'DM Sans','Helvetica Neue',sans-serif" opacity="0.5">
                      LOC
                    </text>
                    <text x="208" y="180" fontSize="9" fontWeight="600" fill="#ffffff"
                      fontFamily="'DM Sans','Helvetica Neue',sans-serif" opacity="0.85">
                      {trunc(location, 18)}
                    </text>
                  </>
            ...
            {salary && (
              <>
                <text x="22" y="195" fontSize="9" fill="#ffffff"
                  fontFamily="'DM Sans','Helvetica Neue',sans-serif" opacity="0.5">
                  INCOME
                </text>
                <text x="65" y="195" fontSize="9" fontWeight="600" fill={theme.accent}
                  fontFamily="'DM Sans','Helvetica Neue',sans-serif" opacity="0.95">
                  {salary}
                </text>
                {company && (
                  <>
                    <text x="180" y="195" fontSize="9" fill="#ffffff"
                      fontFamily="'DM Sans','Helvetica Neue',sans-serif" opacity="0.5">
                      ORG
                    </text>
                    <text x="204" y="195" fontSize="9" fontWeight="600" fill="#ffffff"
                      fontFamily="'DM Sans','Helvetica Neue',sans-serif" opacity="0.85">
                      {trunc(company, 20)}
                    </text>
                  </>
        ...
        <rect x="318" y="185" width="86" height="24" rx="5"
          fill={theme.accent} opacity="0.95" />
        <text x="361" y="200" fontSize="10" fontWeight="700" fill={theme.bg1}
        ...
        <rect x="0" y="215" width="420" height="5" fill={theme.accent} opacity="0.25" />
      </g>
    </svg>
  );
}
...
// ── Ad Posters section ────────────────────────────────────────
function AdPosters({ persona, guestName, meta, analysis }) {
  const [active, setActive] = useState(null);

  if (!persona) return null;

  const variantLabels = {
    1: { title: "Personalised Offer",    desc: "Offer + room recommendation tailored to this guest" },
    2: { title: "Room Recommendation",   desc: "Selected room type based on profile and spending level" },
    3: { title: "Guest Profile Match",   desc: "Persona-matched creative with lifestyle signals" },
  };

  return (
    <div style={{ background:C.surface, borderRadius:10, border:`1px solid ${C.border}`, padding:"22px 26px", marginBottom:18 }}>
      <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:6 }}>
        <span style={{ color:C.textMute, display:"flex" }}>{Icon.Tag(14)}</span>
      ...
      <p style={{ fontSize:12, color:C.textMute, marginBottom:18 }}>
        Personalised ad creatives generated from this guest's profile, persona, and spending signals. Each variant targets a different message angle.
      </p>

      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:16 }}>
        {[1, 2, 3].map(v => (
          <div key={v}
            style={{ borderRadius:10, overflow:"hidden", border:`2px solid ${active === v ? C.text : C.border}`, transition:"all 0.15s", cursor:"pointer" }}
            onClick={() => setActive(active === v ? null : v)}
            onMouseEnter={e => { if (active !== v) e.currentTarget.style.borderColor = C.borderDark; }}
            onMouseLeave={e => { if (active !== v) e.currentTarget.style.borderColor = C.border; }}
          >
            <AdPoster
              variant={v}
              guestName={guestName}
            ...
            <div style={{ padding:"12px 14px", background:C.bg, borderTop:`1px solid ${C.border}` }}>
              ...
              <div style={{ display:"flex", gap:8, marginTop:10 }}>
                <button
                  onClick={e => { e.stopPropagation(); setActive(active === v ? null : v); }}
                  style={{ flex:1, padding:"6px 0", fontSize:11, fontWeight:600, borderRadius:5, border:`1px solid ${C.border}`, background: active === v ? C.fill : C.surface, color: active === v ? "#fff" : C.textMid, cursor:"pointer", fontFamily:"inherit", transition:"all 0.15s" }}>
                  {active === v ? "Selected" : "Select"}
                </button>
                <button
                  onClick={e => { e.stopPropagation(); downloadPoster(v, persona, guestName, meta, analysis); }}
                  style={{ padding:"6px 10px", fontSize:11, fontWeight:600, borderRadius:5, border:`1px solid ${C.border}`, background:C.surface, color:C.textMid, cursor:"pointer", fontFamily:"inherit", display:"flex", alignItems:"center", gap:4, transition:"all 0.15s" }}
                  onMouseEnter={e => { e.target.style.background = C.bg; }}
                  onMouseLeave={e => { e.target.style.background = C.surface; }}
                >
                  {Icon.Download(11)} Save
                </button>
              </div>
            </div>
          </div>
      ...
      {active && (
        <div style={{ marginTop:16, padding:"12px 16px", background:C.bg, borderRadius:8, border:`1px solid ${C.border}`, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <span style={{ fontSize:12, color:C.textMid }}>
            <span style={{ fontWeight:600, color:C.text }}>Variant {active}</span> selected — {variantLabels[active].title}
          </span>
          <span style={{ fontSize:11, color:C.textMute }}>Ready to show to guest</span>
        </div>
      )}
    </div>
  );
...
// ── Download SVG helper ───────────────────────────────────────
function downloadPoster(variant, persona, guestName, meta, analysis) {
  const personaKey = (persona || "").toLowerCase().replace(" traveler", "").trim();
  const theme = PERSONA_THEME[personaKey] || PERSONA_THEME.default;
  const profession = meta?.profession || meta?.jobTitle || "";
  const company    = meta?.company    || meta?.jobCompany || "";
  ...
  const variants = {
    1: { eyebrow:"PERSONALISED OFFER",   headline: trunc(analysis?.personalizedOffer || "Exclusive Guest Package", 55), sub: trunc(analysis?.roomRecommendation || "", 60), cta:"View Offer", tag: theme.label, showProfile:true  },
    2: { eyebrow:"ROOM RECOMMENDATION",  headline: trunc(analysis?.roomRecommendation || "Premium Room Selected", 48), sub: trunc(analysis?.staffNote || "", 70),           cta:"Book Now",   tag: spending ? spending.toUpperCase() + " TIER" : theme.label, showProfile:false },
    3: { eyebrow:"GUEST PROFILE MATCH",  headline: trunc(persona || "Guest Persona", 40),                             sub: lifestyle.length > 0 ? "Interests: " + lifestyle.join("  ·  ") : trunc(analysis?.staffNote || "", 60), cta:"View Profile", tag: seniority ? seniority.toUpperCase() : theme.label, showProfile:true },
  };
  const v = variants[variant] || variants[1];

  const svgContent = `<svg viewBox="0 0 420 220" xmlns="http://www.w3.org/2000/svg" width="840" height="440">
  <defs>
    <linearGradient id="grad" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" stop-color="${theme.bg1}"/>
      <stop offset="100%" stop-color="${theme.bg2}"/>
    </linearGradient>
    <clipPath id="clip"><rect width="420" height="220" rx="10"/></clipPath>
  </defs>
  <g clip-path="url(#clip)">
    <rect width="420" height="220" fill="url(#grad)"/>
    <circle cx="340" cy="30" r="90" fill="${theme.accent}" opacity="0.07"/>
    <circle cx="380" cy="120" r="55" fill="${theme.accent}" opacity="0.05"/>
    <circle cx="20" cy="160" r="70" fill="${theme.accent}" opacity="0.06"/>
    <rect x="0" y="0" width="4" height="220" fill="${theme.accent}" opacity="0.9"/>
    <rect x="320" y="14" width="${v.tag.length * 7 + 18}" height="20" rx="4" fill="${theme.accent}" opacity="0.18"/>
    <text x="329" y="27" font-size="8" font-weight="700" fill="${theme.accent}" font-family="Helvetica Neue,sans-serif" letter-spacing="0.08em">${v.tag}</text>
    <text x="22" y="38" font-size="8" font-weight="700" fill="${theme.accent}" font-family="Helvetica Neue,sans-serif" letter-spacing="0.12em" opacity="0.9">${v.eyebrow}</text>
    <line x1="22" y1="44" x2="200" y2="44" stroke="${theme.accent}" stroke-width="0.5" opacity="0.3"/>
    <text x="22" y="68" font-size="17" font-weight="700" fill="#ffffff" font-family="Helvetica Neue,sans-serif" letter-spacing="-0.3px">${trunc(v.headline, 28)}</text>
    ${v.headline.length > 28 ? `<text x="22" y="90" font-size="17" font-weight="700" fill="#ffffff" font-family="Helvetica Neue,sans-serif" letter-spacing="-0.3px">${trunc(v.headline.slice(28), 28)}</text>` : ""}
    ${v.sub ? `<text x="22" y="122" font-size="10" fill="#ffffff" font-family="Helvetica Neue,sans-serif" opacity="0.65">${trunc(v.sub, 52)}</text>` : ""}
    ${v.showProfile && guestName ? `<line x1="22" y1="150" x2="398" y2="150" stroke="#ffffff" stroke-width="0.4" opacity="0.12"/>
    <text x="22" y="165" font-size="9" fill="#ffffff" font-family="Helvetica Neue,sans-serif" opacity="0.5">GUEST</text>
    <text x="60" y="165" font-size="9" font-weight="600" fill="#ffffff" font-family="Helvetica Neue,sans-serif" opacity="0.85">${trunc(guestName, 20)}</text>
    ${profession ? `<text x="180" y="165" font-size="9" fill="#ffffff" font-family="Helvetica Neue,sans-serif" opacity="0.5">ROLE</text><text x="208" y="165" font-size="9" font-weight="600" fill="#ffffff" font-family="Helvetica Neue,sans-serif" opacity="0.85">${trunc(profession, 18)}</text>` : ""}
    ${salary ? `<text x="22" y="195" font-size="9" fill="#ffffff" font-family="Helvetica Neue,sans-serif" opacity="0.5">INCOME</text><text x="65" y="195" font-size="9" font-weight="600" fill="${theme.accent}" font-family="Helvetica Neue,sans-serif" opacity="0.95">${salary}</text>` : ""}` : ""}
    <rect x="318" y="185" width="86" height="24" rx="5" fill="${theme.accent}" opacity="0.95"/>
    <text x="361" y="200" font-size="10" font-weight="700" fill="${theme.bg1}" font-family="Helvetica Neue,sans-serif" text-anchor="middle" letter-spacing="0.04em">${v.cta} →</text>
    <rect x="0" y="215" width="420" height="5" fill="${theme.accent}" opacity="0.25"/>
  </g>
</svg>`;
  ...
  a.href     = url;
  a.download = "ad_variant_" + variant + "_" + (guestName || "guest").replace(/\s+/g, "_").toLowerCase() + ".svg";
  a.click();
