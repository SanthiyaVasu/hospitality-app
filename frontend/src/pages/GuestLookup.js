...
// Taglines per persona — used as fallback only
...
function HotelAdPoster({guestName,persona,meta,analysis,variant,hotelRecommendations}){
  ...
  // ✅ Use REAL hotel name from hotelRecommendations based on city
  // Pick different hotels for different variants
  const hotels      = hotelRecommendations || [];
  const hotelIndex  = variant - 1; // variant 1→hotels[0], 2→hotels[1], 3→hotels[2]
  const hotelData   = hotels[hotelIndex] || hotels[0];
  const hotelName   = hotelData?.name || "Premium Hotel";
  const hotelAddr   = hotelData?.address && hotelData.address !== location
    ? hotelData.address : location;

  const variants = {
    1:{
      greeting: `Hey ${firstName},`,
      headline1:"We Found Your",
      headline2:"Perfect Hotel.",
      hotelName,
      tagline,
      offer: trunc(analysis?.personalizedOffer||"Exclusive welcome package awaits you",52),
      cta:   "Claim Your Offer",
      badge: theme.label,
    },
    2:{
      greeting: `${firstName}, You Deserve`,
      headline1:"The Best Room",
      headline2:"In The House.",
      hotelName,
      tagline: trunc(analysis?.roomRecommendation||tagline,44),
      offer:   trunc(analysis?.staffNote||"Our staff is ready to make your stay unforgettable",52),
      cta:     "Book Your Room",
      badge:   spending?spending.toUpperCase():theme.label,
    },
    3:{
      greeting: `Exclusively For You,`,
      headline1:firstName+",",
      headline2:"Your Stay Is Ready.",
      hotelName,
      tagline,
      offer: trunc(analysis?.personalizedOffer||"Special offer crafted just for your profile",52),
    ...
    <svg viewBox="0 0 440 260" xmlns="http://www.w3.org/2000/svg"
      ...
      <defs>
        ...
        <clipPath id={`clip_${variant}`}>
          <rect width="440" height="260" rx="10"/>
        </clipPath>
      </defs>
      <g clipPath={`url(#clip_${variant})`}>
        ...
        <text x="366" y="27" fontSize="9" fontWeight="700" fill={theme.accent}
          fontFamily="'DM Sans','Helvetica Neue',sans-serif"
          textAnchor="middle" letterSpacing="0.12em" opacity="0.9">
          {v.badge}
        </text>
        {/* Greeting */}
        <text x="18" y="56" fontSize="11" fontWeight="500" fill={theme.accent}
          fontFamily="'DM Sans','Helvetica Neue',sans-serif" opacity="0.85"
          fontStyle="italic">
          {v.greeting}
        </text>
        {/* Headline */}
        <text x="18" y="82" fontSize="22" fontWeight="800" fill="#ffffff"
          fontFamily="'DM Sans','Helvetica Neue',sans-serif" letterSpacing="-0.5px">
          {v.headline1}
        </text>
        <text x="18" y="108" fontSize="22" fontWeight="800" fill={theme.accent}
          fontFamily="'DM Sans','Helvetica Neue',sans-serif" letterSpacing="-0.5px">
          {v.headline2}
        </text>
        {/* Divider */}
        ...
        {/* Hotel name */}
        <text x="18" y="138" fontSize="13" fontWeight="700" fill="#ffffff"
          fontFamily="'DM Sans','Helvetica Neue',sans-serif" opacity="0.95">
          {trunc(v.hotelName, 36)}
        </text>
        {/* Hotel tagline */}
        <text x="18" y="153" fontSize="10" fontWeight="400" fill="#ffffff"
        ...
        <text x="28" y="177" fontSize="8" fontWeight="700" fill={theme.accent}
          fontFamily="'DM Sans','Helvetica Neue',sans-serif"
          letterSpacing="0.1em" opacity="0.8">
          SPECIAL OFFER
        </text>
        <text x="28" y="191" fontSize="9.5" fill="#ffffff"
          fontFamily="'DM Sans','Helvetica Neue',sans-serif" opacity="0.75">
          {trunc(v.offer, 46)}
        </text>
        {/* Location — always shows real city */}
        <text x="18" y="222" fontSize="9" fontWeight="500" fill="#ffffff"
          fontFamily="'DM Sans','Helvetica Neue',sans-serif" opacity="0.55"
          letterSpacing="0.05em">
          {"\uD83D\uDCCD"} {trunc(hotelAddr, 28)}
        </text>
        {/* CTA Button */}
        <rect x="308" y="218" width="116" height="30" rx="6" fill={theme.accent} opacity="0.95"/>
        <text x="366" y="237" fontSize="10" fontWeight="700" fill={theme.bg1}
          fontFamily="'DM Sans','Helvetica Neue',sans-serif"
...
function downloadSVG(svgEl,filename){
  ...
  const b   = new Blob([str],{type:"image/svg+xml"});
...
// ── Ad Posters Section ────────────────────────────────────────
function AdPosters({persona,guestName,meta,analysis,hotelRecommendations}){
  ...
  return(
    <div style={{background:C.surface,borderRadius:10,border:`1px solid ${C.border}`,padding:"22px 26px",marginBottom:18}}>
      ...
      <p style={{fontSize:12,color:C.textMute,marginBottom:20}}>
        Personalised hotel advertisement posters generated from this guest's profile. Hotels are matched to guest's actual city. Click to select, then send to guest.
      </p>
            ...
            <div style={{padding:"12px 14px",background:C.bg,borderTop:`1px solid ${C.border}`}}>
              ...
              <div style={{display:"flex",gap:8}}>
                ...
                <button
                  onClick={e=>{
                    e.stopPropagation();
                    const svgEl=refs[v].current?.querySelector("svg");
                    if(svgEl) downloadSVG(svgEl,`hotel_ad_v${v}_${(guestName||"guest").replace(/\s+/g,"_").toLowerCase()}.svg`);
                  }}
      ...
      {selected&&(
        <div style={{marginTop:14,padding:"11px 16px",background:C.bg,borderRadius:8,border:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <span style={{fontSize:12,color:C.textMid}}>
            <span style={{fontWeight:700,color:C.text}}>Variant {selected}</span> — {labels[selected].title} — ready to send to guest
          </span>
          <span style={{fontSize:11,color:C.textMute}}>✓ Selected</span>
        </div>
      )}
    </div>
  );
...
function Card({children,style={}}){return <div style={{background:C.surface,borderRadius:10,border:`1px solid ${C.border}`,padding:"22px 26px",...style}}>{children}</div>;}
...
function Pill({children}){return <span style={{display:"inline-block",padding:"3px 10px",borderRadius:4,fontSize:11,fontWeight:500,background:C.tag,color:C.textMid,border:`1px solid ${C.tagBorder}`,margin:"0 4px 4px 0"}}>{children}</span>;}
  ...
  return (
    <Card style={{ marginBottom:18 }}>
      ...
      <p style={{ fontSize:12, color:C.textMute, marginBottom:18, marginTop:-10 }}>
        Show this QR code to the guest. They scan it with their phone and fill their preferences — data saves automatically.
      </p>
      <div style={{ display:"flex", gap:32, alignItems:"flex-start" }}>
        ...
        <div style={{ flex:1 }}>
          ...
          {[
            "Guest scans QR code with their phone camera",
            "Preference form opens pre-filled with name & email",
            "Guest selects room type, amenities, activities",
            "Data saves directly to your database",
          ].map((step, i) => (
            <div key={i} style={{ display:"flex", gap:10, alignItems:"flex-start", marginBottom:10 }}>
              <div style={{ width:20, height:20, borderRadius:"50%", background:C.text, color:"#fff", fontSize:10, fontWeight:700, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, marginTop:1 }}>
                {i+1}
              </div>
              <span style={{ fontSize:12, color:C.textMid, lineHeight:1.5 }}>{step}</span>
            </div>
          ))}
          ...
          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
            <div style={{ flex:1, padding:"8px 12px", background:C.bg, border:`1px solid ${C.border}`, borderRadius:6, fontSize:11, color:C.textMid, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
              {formUrl}
            </div>
            <button onClick={copyLink}
              style={{ padding:"8px 14px", borderRadius:6, border:`1px solid ${C.border}`, background: copied ? C.fill : C.bg, color: copied ? "#fff" : C.textMid, fontSize:11, fontWeight:600, cursor:"pointer", fontFamily:"inherit", whiteSpace:"nowrap", transition:"all 0.15s" }}>
              {copied ? "Copied!" : "Copy Link"}
            </button>
            <a href={formUrl} target="_blank" rel="noreferrer"
              style={{ padding:"8px 14px", borderRadius:6, border:`1px solid ${C.border}`, background:C.bg, color:C.textMid, fontSize:11, fontWeight:600, textDecoration:"none", whiteSpace:"nowrap" }}>
              Open
            </a>
          </div>
        </div>
