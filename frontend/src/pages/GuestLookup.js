...
const PERSONA_THEME = {
  ...
  sports:    {bg1:"#0d1a0d",bg2:"#2e3c2d",accent:"#6cbf5b",label:"SPORTS"},
...
export default function GuestLookup() {
  ...
  async function handleSendEmail() {
    ...
    try {
      ...
      // Generate SVG string for the selected variant
      const personaKey = (persona||"").toLowerCase().replace(" traveler","").trim();
      const themes = {
      ...
      const staffNote = analysis?.staffNote         || "";
      ...
      const svgString = `<svg viewBox="0 0 420 220" xmlns="http://www.w3.org/2000/svg" width="840" height="440">
<defs>
...
<g clip-path="url(#c)">
  ...
  <text x="18" y="38" font-size="11" font-weight="500" fill="${t.accent}" font-family="Helvetica Neue,sans-serif" opacity="0.85" font-style="italic">Hey ${(guestName||"Guest").split(" ")[0]},</text>
  ...
  <text x="18" y="194" font-size="9" fill="#ffffff" font-family="Helvetica Neue,sans-serif" opacity="0.5">📍 ${loc}</text>` : ""}
